{-|
Module      : OpenQASM
Description : Simulate OpenQASM with path sums
...
module Feynman.Simulation.OpenQASM where
...
import Feynman.Algebra.Frontend.OpenQASM.Syntax

-- | Simulation state.
--
--   Simulation proceeds by keeping track of the state in a path sum
--   which represents the operator/state at the current point, which
--   can either be a pure-state/operator in a single rail, i.e.
--
--               ----------U----------
--
--  or a density matrix/operator represented in CPM or dual-rail style
--
--               ----------U*--------
--               ----------U---------
...
-- | Lookup a variable
lookupDec :: ID -> State SimState Record
lookupDec id = do
...
alloc dec@(VarDec id typ) = case typ of
  Qreg n -> do
    idx <- gets (outDeg . state)
    modify (\st -> st { state = state st <> identity n })
    return $ ArrayRec dec (Map.fromList $ zip [0..n-1] [idx..idx + n - 1])
  Creg n -> do
    idx <- gets (outDeg . state)
    modify (\st -> st { state = state st <> identity n })
    return $ ArrayRec dec (Map.fromList $ zip [0..n-1] [idx..idx + n - 1])
  
-- | Simulates a QASM AST
simulateQASM :: QASM -> SimState
simulateQASM (QASM ver xs) = execState (sequence_ goStmt xs) initSt where

  goStmt stmt = case stmt of
    IncStmt _       -> return ()
    DecStmt dec     -> goDec dec
    QStmt qexp      -> goQExp qexp
    IfStmt x i qexp -> do
      iarr <- loopupArg (Var x)
      controlledIDX iarr

  goDec dec = case dec of
    VarDec var   -> do
      record <- alloc var
      addDec (id var) record
    GateDec gate -> do
      addDec (id gate) (GateRec gate)
    UIntDec _  -> error "Can't simulate opaque gates!"

  goQExp qexp = case qexp of
    GateExp uexp     -> goUExp uexp
    MeasureExp a1 a2 -> do
      i1 <- lookupArg a1
      i2 <- lookupArg a2
      measureIDX i1
...
-- | Maps an access path to a unique ID
