...
-- Watch out, aliasing "tof" to "X" by recursing is a little sketchy, but the
-- REAL hazard is how "X" handling calls into "mct" which generates more "X"
-- gates and processes them back in "gateToCliffordT"! If the 1- and 2-param
-- "X" or "tof" generated by "mct" are allowed to recurse back into
-- "gateToCliffordT", There Will Be Problems, i.e. divergence
...
gateToCliffordT (Gate g i p) =
  let circ = case (g, p) of
        ...
        ("cz", [x,y])   -> [CZ x y]
        ("swap", [x,y]) -> [Swap x y]
        ("X", [x,y,z])  -> [H z, T x, T y, CNOT z x, Tinv x, CNOT y z, Tinv z,
        ...
        ("Zd", [x,y,z]) -> [Tinv x, Tinv y, CNOT z x, T x, CNOT y z, T z,
                            ...
                            Tinv z, CNOT y x]
        ("tof", _)      -> gateToCliffordT (Gate "X" i p)
...
gateFromExtractionBasis g = case g of
  ...
  Phase f xs    -> phaseFromExtractionBasis f xs
  Swapper x y   -> [Gate "swap" 1 [x, y]]
  where
    phaseFromExtractionBasis theta xs
      | a == 0 = []
      | n == 0 = [Gate "Z" 1 xs]
      | n == 1 = case a `mod` 4 of
          1 -> [Gate "S" 1 xs]
      ...
      | n == 2 = case a `mod` 8 of
          1 -> [Gate "T" 1 xs]
          3 -> [Gate "T*" 1 xs, Gate "Z" 1 xs]
          5 -> [Gate "T" 1 xs, Gate "Z" 1 xs]
          7 -> [Gate "T*" 1 xs]
      | otherwise = [ParamGate "Rz" 1 (Discrete theta) xs]
      where (Dy a n) = unpack theta

fromExtractionBasis :: [ExtractionGates] -> [Gate]
fromExtractionBasis = concatMap gateFromExtractionBasis
...
decompileDotQC qc = qc { decls = resynthesizedDecls, qubits = Set.toList (Set.union (inputs qc) usedIDs) }
  where -- The resynthesis algorithm uses a bunch of ancillas, but doesn't
        ...
        go decl =
          let circuitQubits  = qubits qc ++ params decl
              circuitInputs  = Set.toList (inputs qc) ++ params decl
              resynthesize c = traceResynthesis ("decompile, inputs: " ++ show circuitInputs ++ ", qubits: " ++ show circuitQubits) $
                case doResynthesizeCircuit $ toCliffordT c of
                  Nothing -> traceResynthesis "Resynthesis failed, returning original circuit" c
