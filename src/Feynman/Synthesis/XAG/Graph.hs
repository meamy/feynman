...
--   freeVariables :: g -> IntSet
--   internal :: g -> IntSet
--   coverIDs :: IntSet -> g -> IntSet
--   coverIDs ids = (map nodeID) . (coverNodes ids)
--   coverNodes :: IntSet -> g -> [Node]
...
mergeAt newInputIDs leftG leftInputIDs outIndex rightG rightInputIDs =
  ...
  where
    ...
    (leftRenumberedNodes, leftNextID, leftNodesMap) =
      renumberNodes leftInputsMap firstNodeID (nodes leftG)
    ...
    renumberNodes idMap nextID (node : remain) =
      ...
      where
        (renumberedRemain, finalNextID, finalMap) =
          renumberNodes (IntMap.insert (nodeID node) nextID idMap) (nextID + 1) remain

    renumberNode idMap nextID node@(Const nID val) = Const nextID val
    renumberNode idMap nextID node@(Not nID xID) = Not nextID (idMap IntMap.! xID)
    renumberNode idMap nextID node@(Xor nID xID yID) =
      Xor nextID (idMap IntMap.! xID) (idMap IntMap.! yID)
    renumberNode idMap nextID node@(And nID xID yID) =
      And nextID (idMap IntMap.! xID) (idMap IntMap.! yID)

    firstNodeID = (foldl max 2 newInputIDs) + 1
    rightInputsMap = IntMap.fromList (zip (inputIDs rightG) rightInputIDs)
    leftInputsMap = IntMap.fromList (zip (inputIDs leftG) leftInputIDs)
...
dominatingAnds ofID allNodes = dominatingAndsOfID (IntSet.singleton ofID) (skipToNode allNodes)
  where
    ...
    dominatingAndsOfID :: IntSet -> [Node] -> [(Int, (Int, Int))]
