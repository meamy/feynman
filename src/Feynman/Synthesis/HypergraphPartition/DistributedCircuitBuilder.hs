...
verifyDist circuit initialPartitions = go circuit Map.empty Map.empty
  where
    -- Fixed data-qubit partitions from KaHyPar (never mutated)
    partOf :: ID -> Maybe Int
    partOf q = Map.lookup q initialPartitions

    isBell :: ID -> Bool
    isBell q = any (`isPrefixOf` q) ["bellL", "bellA", "bellB"]

    go :: [Primitive] -> Map ID Int -> Map ID ID -> Bool
    go []       _    _       = True
    go (gate:gs) home partner =
      case gate of

        CNOT c t
          -- 1. Bell-to-Bell Interactions
          | isBell c && isBell t ->
              if Map.notMember c partner && Map.notMember t partner
              then 
                -- Case A: EPR initialization
                go gs home (Map.insert c t (Map.insert t c partner))
                
              else if Map.lookup c partner == Just t then
                -- Case B: Classical Feed-Forward (X-Correction).
                -- Partners are expected to be on opposite QPUs; allowed to cross
                go gs home partner
                
              else 
                -- Case C: Proxy Logic Gate (e.g., inside Gray Synthesis).
                -- Not partners. They MUST physically reside on the same QPU!
                case (Map.lookup c home, Map.lookup t home) of
                  (Just hc, Just ht) | hc /= ht -> False
                  _                             -> go gs home partner

          -- 2. Entangler: Data control onto a Bell target.
          -- The target Bell inherits the Data's QPU; its partner gets the opposite QPU.
          | not (isBell c) && isBell t ->
              case partOf c of
                Nothing -> go gs home partner            -- unknown data qubit
                Just pc ->
                  let home1 = Map.insert t pc home
                      home2 = case Map.lookup t partner of
                                Just tp -> Map.insert tp (1 - pc) home1
                                Nothing -> home1
                  in go gs home2 partner

          -- 3. Core nonlocal gate: Bell control onto a Data target.
          -- A mismatch between the proxy's home and the data's QPU is a real violation.
          | isBell c && not (isBell t) ->
              case (Map.lookup c home, partOf t) of
                (Nothing, _)                -> False     -- core gate on unplaced Bell qubit
                (Just hc, Just pt) | hc /= pt -> False
                _                           -> go gs home partner

          -- 4. Data-data CNOT: Ordinary locality rule.
          | otherwise ->
              case (partOf c, partOf t) of
                (Just pc, Just pt) | pc /= pt -> False
                _                             -> go gs home partner

        CZ c t
          -- Disentangler feedback (a Bell operand) legitimately spans the cut classically.
          | isBell c || isBell t -> go gs home partner
          -- Data-data CZ: standard locality rule.
          | otherwise ->
              case (partOf c, partOf t) of
                (Just pc, Just pt) | pc /= pt -> False
                _                             -> go gs home partner
        ...
        Reset q -> 
          -- Free a Bell qubit: drop its derived home and partnership.
          go gs (Map.delete q home) (Map.delete q partner)

        -- Single-qubit gates are inherently local.
        _ -> go gs home partner
